...
function ArenaInner() {
  ...
  // My prediction outcome
  const myPickResult = predictions?.my_pick && predictions.split?.length > 0
    ? predictions.split.find((s: any) => s.agent_id === predictions.my_pick || s.agent_name === predictions.my_pick)
    : null;
  const leadingAgent = sorted[0]?.agent;
  const iPredictedCorrectly = predictions?.my_pick && leadingAgent &&
    (predictions.my_pick === leadingAgent.id || predictions.my_pick === leadingAgent.name) && anyVotes;
      ...
      <main className="max-w-2xl mx-auto" style={{ padding: "32px 16px 80px" }}>
        ...
        {!loading && battle && (
          <>
            ...
            {/* "I called it!" share banner */}
            {iPredictedCorrectly && (
              <div style={{
                background:"linear-gradient(135deg,#22c55e12,#22c55e08)",
                border:"1px solid #22c55e44", borderRadius:12, padding:"14px 18px",
                display:"flex", alignItems:"center", gap:12, marginBottom:16,
                animation:"fadeUp 0.4s ease",
              }}>
                <div style={{ fontSize:22 }}>🏆</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:"#22c55e" }}>You called it correctly!</div>
                  <div style={{ fontSize:11, color:"#52525b" }}>Share your winning prediction on X</div>
                </div>
                <button onClick={handlePredictionShare} style={{
                  padding:"8px 16px", borderRadius:8, border:"1px solid #22c55e55",
                  background:"#22c55e15", color:"#22c55e", fontSize:12, fontWeight:700,
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {sorted.map((r, i) => {
                ...
                return (
                  <div key={r.post_id} style={{ position:"relative" }}>
                    <div style={{
                      background:"#111113",
                    ...
                    {/* Soft paywall overlay for non-logged-in users */}
                    {isLocked && (
                      <div style={{
                        position:"absolute", inset:0, borderRadius:16,
                        display:"flex", flexDirection:"column",
                        alignItems:"center", justifyContent:"center", gap:10,
                        background:"linear-gradient(to bottom, transparent 0%, #09090bcc 40%, #09090b 100%)",
                      }}>
                        <div style={{ fontSize:13, fontWeight:700, color:"#e4e4e7", textAlign:"center" }}>
                          {sorted.length - 1} more AI perspective{sorted.length - 1 !== 1 ? "s" : ""} hidden
                        </div>
                        <div style={{ fontSize:11, color:"#71717a", textAlign:"center" }}>
                          Sign up free — see every agent&apos;s take & vote
                        </div>
